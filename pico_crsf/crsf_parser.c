...
/*
 * This table is generated from the CRSF CRC-8 generator polynomial (0xD5).
 * It is intentionally generated rather than copied from any existing
 * implementation. To regenerate, run:
 *
 *     python crc8_generator.py
 */
...
void CRSFParser_update(crsf_parser *parser){
    ...
    // they are chained IFs for the edge case in which multiple steps can be completed in one go.
    if(parser->state == WAIT_SYNC){
        int res = ringBuffer_readChar(&parser->buffer);
        if(res >= 0 && res == 0xC8){
    ...
    if(parser->state == READ_LENGTH){
        int res = ringBuffer_readChar(&parser->buffer);
        if(res >= 0){
            if(res <= 62 && res >= 2){
                parser->packetLength = res;
                parser->state = AWAIT_PACKET;
            }
            else {
                parser->state = WAIT_SYNC;
            }
    ...
    if(parser->state == CHECK_CRC){
        if(parser->rawPacket[parser->packetLength - 1] == crc8(parser->rawPacket, parser->packetLength - 1)){
            parser->state = PARSE;
        }
        else {
            parser->state = WAIT_SYNC;
